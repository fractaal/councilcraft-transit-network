...
// Local filesystem cache for transcoded DFPWM. We no longer use cloud storage.
const USE_LOCAL_CACHE = true;
const LOCAL_CACHE_DIR = join(__dirname, "cache");

// WebSocket content agent settings
const PA_USE_AGENT = process.env.PA_USE_AGENT !== "false";
const PA_REQUIRE_AGENT = process.env.PA_REQUIRE_AGENT === "true";
...
// Express-style handler for streaming DFPWM audio.
// Previously exported as a Firebase onRequest function; the Cloud wrapper
// has been removed so this can be mounted directly on an Express app.
export async function paStream(req, res) {
    ...
    try {
      ...
      return;
	    ...
	    await installationReady;
	    	    ...
	    	    // Check cache first (local filesystem only)
	    	    const cacheKey = getCacheKey(trackUrl);
	    	
	    	    try {
	    	      const exists = await checkLocalCache(cacheKey);
	    	      if (exists) {
	    	        log("INFO", "cache_hit", { requestId, trackUrl, cacheKey, mode: "local" });
	    	        res.setHeader("Content-Type", "audio/dfpwm");
	    	        res.setHeader("Cache-Control", "public, max-age=86400");
	    	        res.setHeader("X-Cache", "HIT");
	    	
	    	        const cacheStream = createLocalCacheReadStream(cacheKey);
	    	    ...
	    	    log("INFO", "cache_miss", { requestId, trackUrl, cacheKey, mode: "local" });
	    	
	    let readable = null;
	    let source = "yt_dlp";
	
	    if (PA_USE_AGENT && agentHub.hasAgents()) {
	      try {
	        readable = agentHub.createAudioStream(trackUrl);
	        source = "agent";
	        log("INFO", "agent_stream_selected", { requestId, trackUrl });
	      } catch (error) {
	        log("WARN", "agent_stream_unavailable", {
	          requestId,
	          trackUrl,
	          error: error?.message || String(error),
	        });
	        if (PA_REQUIRE_AGENT) {
	          res.status(503).send("No content retrieval agents available");
	          return;
	        }
	      }
	    }
	    ...
	    if (!readable) {
	      ...
	      readable = resolveReadable(downloadHandle);
	      if (!readable) {
	    ...
	    let bytesSent = 0;
	    log("INFO", "stream_pipeline_start", { requestId, trackUrl, source });
    ...
    // Create PassThrough stream to split output to client and cache
	    ...
	    const cacheWriteStream = createLocalCacheWriteStream(cacheKey);

	    let cacheUploadFailed = false;
	    cacheWriteStream.on("error", (error) => {
	      cacheUploadFailed = true;
	      log("WARN", "cache_upload_failed", { requestId, error: error.message, mode: "local" });
	    });
	    cacheWriteStream.on("finish", () => {
	      if (!cacheUploadFailed) {
	        log("INFO", "cache_upload_completed", { requestId, trackUrl, cacheKey, mode: "local" });
	      }
	    });
...
// Express-style handler for track metadata.
// Previously exported as a Firebase onRequest function.
export async function paInfo(req, res) {
  const requestId = randomUUID();
  ...
  try {
    ...
    return;
	  ...
	  if (PA_USE_AGENT && agentHub.hasAgents()) {
	    try {
	      const agentInfo = await agentHub.requestInfo(trackUrl);
	      const durationSeconds = Number(agentInfo?.durationSeconds || 0) || 0;
	      result = {
	      ...
	      source = "agent";
	      log("INFO", "info_via_agent", { requestId, trackUrl });
	    } catch (error) {
	      log("WARN", "agent_info_unavailable", {
	        requestId,
	        trackUrl,
	        error: error?.message || String(error),
	      });
	  ...
	  try {
	    ...
	    const durationSeconds = Number(result?.durationSeconds || 0) || 0;
	    res.status(200).json({
	      title: result?.title || "",
